#!/bin/bash
# task scheduler fallout
echo task scheduler stuff
set -x
rm -f c:/temp/log.txt
rm -f c:/temp/ts_log.txt
rm -f c:/temp/ts_3rd_party_log.txt
rm -f c:/temp/3rd_party_ts_log.txt
rm -f c:/temp/ts_copy_src_fmt_log.txt
rm -f c:/temp/ts_example[12].pptx
rm -f c:/temp/ts_3rd_example[12].pptx
rm -f c:/temp/example.pptx
rm -f c:/temp/sales_summary.pptx

set +x
echo random logs
set -x
rm -f c:/temp/allsyntax_log.txt
rm -f c:/temp/ansi_log.txt
rm -f c:/temp/bigtest_log.txt
rm -f c:/temp/bom_tests_log_file.txt
rm -f c:/temp/copy_src_fmt_log.txt
rm -f c:/temp/damaged_shapes_log_file.txt
rm -f c:/temp/damaged_shapes_log_file[12].txt
rm -f c:/temp/doc_example[12]_log.txt
rm -f c:/temp/doc_example_log_edited.txt
rm -f c:/temp/file_errs_log.txt
rm -f c:/temp/file_errs_log_edited.txt
rm -f c:/temp/geom_data_log_file.txt
rm -f c:/temp/geom_log.txt
rm -f c:/temp/init_files_log[12345].txt
rm -f c:/temp/init_files_log_edited.txt
rm -f c:/temp/lex_stress_log[123456].txt
rm -f c:/temp/lex_stress_log_edited.txt
rm -f c:/temp/lex_warning_tests_log_file.txt
rm -f c:/temp/log_output.txt
rm -f c:/temp/move_size_tests_log_file.txt
rm -f c:/temp/move_size_tests_log_file_edited.txt
rm -f c:/temp/move_size_tests_log_file[12345678].txt
rm -f c:/temp/protocol_files_log.txt
rm -f c:/temp/slide_oor_log.txt
rm -f c:/temp/slide_size_log_file[12].txt
rm -f c:/temp/max_pics_caps_log.txt
rm -f c:/temp/ole_err_log.txt
rm -f c:/temp/ole_err_log_edited.txt
rm -f c:/temp/utf8_log.txt
rm -f c:/temp/v13pics_data_log_file.txt
rm -f c:/temp/v13caps_data_log_file.txt
rm -f c:/temp/v19_log.txt
rm -f c:/temp/v24log.txt
rm -f c:/temp/v26log.txt
rm -f c:/temp/v27_cat[123]_log.txt
rm -f c:/temp/v27_cat_log_edited.txt
rm -f c:/temp/v30_log_edited.txt
rm -f c:/temp/v30_log.txt
rm -f c:/temp/v30_log_1[abcd].txt
rm -f c:/temp/v30_fix_1_log.txt c:/temp/v30_fix_1_log_edited.txt
rm -f c:/temp/v30_fix_2[ab]_log.txt
rm -f c:/temp/v30_fix_2a_log_edited.txt
rm -f c:/temp/v30_fix_3_log.txt c:/temp/v30_fix_3_log_edited.txt
rm -f c:/temp/v31_log.txt
rm -f c:/temp/v31_win10_log.txt
rm -f c:/temp/v32_log[012345678].txt
rm -f c:/temp/v32_cmdline_log[12].txt
rm -f c:/temp/v33_log[12345678].txt
rm -f c:/temp/v33_log_edited.txt
rm -f c:/temp/run*_log.txt

set +x
echo random presentations
set -x
rm -f c:/temp/delete_me.html
rm -f c:/temp/delete_me.pptx
rm -f c:/temp/delete_me.pdf
rm -rf c:/temp/delete_me_files
rm -f c:/temp/demo.potx
rm -f c:/temp/mytmplt.pptx
rm -f c:/temp/slides.pptx
rm -f c:/temp/junk.pptx
rm -f c:/temp/max_caps_pics_result.pptx
rm -f c:/temp/pic_geom.pptx
rm -f c:/temp/v27_cat1_[1234].pptx
rm -f c:/temp/v27_cat2_[1234].pptx
rm -f c:/temp/v27_cat3_[1234].pptx
rm -f c:/temp/must_be_pptx_fmt.pptx

set +x
echo random stuff
set -x
rm -f c:/temp/*.cvr
rm -rf c:/temp/*.tmp
rm -f c:/temp/*.od
rm -f c:/temp/*.log
rm -f c:/temp/v33_root_script_edited.txt

echo
